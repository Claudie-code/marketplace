...
export const selectCartTotal = createSelector([getItems, selectDeliveryCost], (items, deliveryCost) => items.map(item => item.price?.$numberDecimal * item.quantity).reduce(calculateTotal, 0) + deliveryCost) 